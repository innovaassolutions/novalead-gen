...
export default function LinkedInAdsScraper() {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold">LinkedIn Ads Scraper</h1>
        <p className="text-muted-foreground">Find companies running ad campaigns on LinkedIn</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <ScraperConfigForm scraperType="linkedin_ads" onStarted={setActiveRunId} />

        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Active Runs</h2>
          {activeRunId && <ScraperProgress runId={activeRunId} />}
          {activeRuns?.filter(r => r.type === "linkedin_ads").map((run) => (
            <ScraperProgress key={run._id} runId={run._id} />
          ))}
          {!activeRunId && (!activeRuns || activeRuns.filter(r => r.type === "linkedin_ads").length === 0) && (
